import { FoamWorkspace, IDisposable, URI } from '../index';
import { Resource } from './note';

export type TagMetadata = { title: string; uri: URI };
...
export class FoamTags implements IDisposable {
  ...
  updateTagsForUpdatedResource(oldVersion: Resource, newVersion: Resource) {
    ...
    this.resolveUpdatedTitles(newVersion, oldVersion);
  ...
  resolveResource(resource: Resource): void {
    resource.tags.forEach(tag => {
      ...
      this.tags[tag].push({ title: resource.title, uri: resource.uri });
    });
  }
  ...
  /**
   * See if the titles are updated. If so, update all relevant metadata objects of the tag with the altered tags
   *
   * @param newVersion
   * @param oldVersion
   */
  private resolveUpdatedTitles(newVersion: Resource, oldVersion: Resource) {
    if (newVersion.title !== oldVersion.title) {
      newVersion.tags.forEach(tag => {
        this.tags[tag].forEach((meta, idx) => {
          if (URI.isEqual(meta.uri, newVersion.uri)) {
            this.tags[tag][idx].title = newVersion.title;
          }
        });
      });
    }
  ...
  private resolveMovedResource(newVersion: Resource, oldVersion: Resource) {
    if (
      ...
      newVersion.tags.forEach(tag => {
        this.tags[tag].forEach((meta, idx) => {
          if (meta.title === newVersion.title) {
  ...
  private resolveUpdatedTags(newVersion: Resource, oldVersion: Resource) {
    ...
    newTags.forEach(newTag => {
      ...
      this.tags[newTag].push({ title: newVersion.title, uri: newVersion.uri });
    });
    ...
    if (
      ...
      newVersion.tags.forEach(tag => {
        ...
        console.log(noteRegistered);
        if (!noteRegistered) {
          this.tags[tag].push({ title: newVersion.title, uri: newVersion.uri });
        }
      });
    }
  }
