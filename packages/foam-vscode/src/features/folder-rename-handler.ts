...
export class FolderRenameHandler {
  ...
  async handleFolderRename(
    ...
    const newPathPrefix = fromVsCodeUri(newUri).path;
    let suppressedErrorCount = 0;

    Logger.info(
      `Starting rename from "${vscode.workspace.asRelativePath(
        oldUri
    ...
    Logger.error = (message: any, ...params: any[]) => {
      ...
      if (
        ...
        const pathPartRegex =
          /FileDataStore: error while reading uri: (.*?)\s+-/;
        const pathMatch = fullMessage.match(pathPartRegex);
        const erroredPath =
          pathMatch && pathMatch[1] ? pathMatch[1].trim() : null;

        if (
          erroredPath &&
          (erroredPath.startsWith(oldPathPrefix) ||
            erroredPath.startsWith(newPathPrefix)) &&
          (fullMessage.includes('ENOENT') ||
            fullMessage.includes('EntryNotFound'))
        ) {
          suppressedErrorCount++;
          Logger.debug(
            `Suppressed expected FileDataStore error for rename paths: ${fullMessage}`
          );
          return;
        }
      }
    ...
    try {
      ...
      const mode = getFoamVsCodeConfig<string>(
        'folderRename.updateLinks',
        'confirm'
      );
      const maxFiles = getFoamVsCodeConfig<number>(
      ...
      try {
        Logger.debug(
          'Refreshing Foam workspace before discovering markdown files...'
        );
        await this.foam.services.matcher.refresh();
        await this.foam.graph.update();
        Logger.debug('Initial workspace refresh complete');
      } catch (refreshError) {
        Logger.warn(
          'Initial workspace refresh failed, proceeding with discovery',
          refreshError
        );
      }
      ...
      if (markdownFilesInFolder.length === 0) {
        Logger.info(
          `No markdown files found in renamed folder: ${vscode.workspace.asRelativePath(
            oldUri
          )}`
        );
        return result;
      ...
      Logger.debug(
        ...
        } markdown files in "${vscode.workspace.asRelativePath(oldUri)}".`
      );
      ...
      // First, refresh the workspace state so we have accurate information about file locations
      Logger.debug(
      ...
      try {
        ...
        for (const resource of resourcesToRemove) {
          try {
            ...
            Logger.debug(
              `Pre-removed old file URI from workspace: ${resource.uri.path}`
            );
          } catch (deleteError) {
            Logger.warn(
              `Failed to pre-remove old file URI ${resource.uri.path} from workspace:`,
              deleteError
            );
          }
        }
        ...
        for (const fileUri of newFilesToAdd) {
          try {
            ...
            Logger.debug(
              `Pre-added new file URI to workspace: ${fileUri.path}`
            );
          } catch (addError) {
            Logger.warn(
              `Failed to pre-add new file URI ${fileUri.path} to workspace:`,
              addError
            );
          }
        }

        // Refresh matcher and graph
        await this.foam.services.matcher.refresh();
        await this.foam.graph.update();

        Logger.debug('Pre-refresh complete');
      } catch (preRefreshError) {
        Logger.warn(
          'Pre-refresh failed, continuing with calculation:',
          preRefreshError
        );
      }
      ...
      try {
        ...
        Logger.debug(
          `Dry run complete: ${dryRunCalcResult.linksUpdatedCount} potential links in ${dryRunCalcResult.filesToEditCount} files.`
        );
        if (dryRunCalcResult.calculationErrors.length > 0) {
          Logger.warn(
            'Dry run encountered errors:',
            dryRunCalcResult.calculationErrors.join('\n')
          );
          result.warnings.push(
            ...dryRunCalcResult.calculationErrors.map(e => `Dry run: ${e}`)
          );
        }
      } catch (dryRunError) {
      ...
      if (mode === 'always' && !options.confirmAction) {
        ...
        if (userDecision === 'settings') {
          ...
          try {
            ...
            vscode.window.showInformationMessage(
              `Folder rename aborted while browsing settings. Reverted '${vscode.workspace.asRelativePath(
                newUri
              )}' back to '${vscode.workspace.asRelativePath(oldUri)}'. ` +
                `You can rename the folder again after adjusting your settings.`
            );
            return result;
          } catch (revertError) {
            Logger.error(
              'Failed to revert folder rename after opening settings:',
              revertError
            );
            result.errors.push(
              `Failed to revert folder rename after opening settings: ${revertError}`
            );
            vscode.window.showErrorMessage(
              `Failed to revert folder rename: ${revertError}. You can change folder rename behavior in the Foam settings that just opened.`
            );
            return result;
          }
        }
        ...
        if (userDecision === 'abort') {
          ...
          try {
            await this.revertFolderRename(newUri, oldUri);
            vscode.window.showInformationMessage(
              `Folder rename aborted. Reverted '${vscode.workspace.asRelativePath(
                newUri
              )}' back to '${vscode.workspace.asRelativePath(oldUri)}'.`
            );
            return result;
            ...
            result.errors.push(
              `Failed to revert folder rename: ${revertError}`
            );
            vscode.window.showErrorMessage(
              `Failed to revert folder rename: ${revertError}`
            );
      ...
      if (userDecision === 'cancel') {
        ...
        if (dryRunCalcResult && dryRunCalcResult.edits.size > 0) {
          ...
          Logger.debug(
            'No link updates were needed based on the dry run, or no edits to apply.'
          );
          result.filesProcessed = dryRunCalcResult.filesProcessedCount;
        } else {
          // This case should ideally not be hit if dryRunCalcResult is always available before this point
          Logger.warn(
            'Proceeding with link updates, but dry run data was not available. This is unexpected.'
          );
        ...
        Logger.error(
          'Unexpected state: userDecision is undefined when it should be defined.'
        );
        result.errors.push(
          'Internal error: Could not determine user action for folder rename.'
        );
        // Proceed to cache refresh, but with an error logged.
      }

      Logger.debug('Final workspace and graph update...');
      try {
        // Since we already updated the workspace before calculating edits,
        // we just need a final graph update to ensure everything is consistent
        await this.foam.graph.update();

        await new Promise(resolve => setTimeout(resolve, 100));

        Logger.info('Foam workspace, graph, and matcher updated successfully.');
      } catch (refreshError) {
        const refreshErrorMessage = `Failed to perform final workspace update: ${refreshError}`;
        Logger.warn(`${refreshErrorMessage}`, refreshError);
        result.warnings.push(
          'Final update failed - some links may appear broken until VS Code restart or manual Foam rescan.'
        );
      }

      if (suppressedErrorCount > 0) {
        Logger.debug(
          `Suppressed ${suppressedErrorCount} expected FileDataStore errors during the rename operation.`
        );
      }
      Logger.info(
        `Successfully completed rename operation from "${vscode.workspace.asRelativePath(
          oldUri
        )}" to "${vscode.workspace.asRelativePath(newUri)}".`
      );
      return result;
  ...
  private async processFilesForLinkUpdates(
    ...
    progress?: vscode.Progress<{ message?: string; increment?: number }>
  ): Promise<ProcessFilesInternalResult> {
    // Changed return type
    Logger.debug(
      `Calculating link updates for ${markdownFiles.length} file(s) in parallel...`
    );
    if (progress) {
      progress.report({
        message: `Analyzing ${markdownFiles.length} files for link updates...`,
      });
    }
    ...
    const fileProcessingPromises = markdownFiles.map(async oldFileUri => {
      ...
      try {
        ...
        Logger.debug(
          `Checking backlinks for: ${vscode.workspace.asRelativePath(
            oldFileUri
          )} (new: ${vscode.workspace.asRelativePath(newFileUri)})`
        );

        const connections = this.foam.graph.getBacklinks(
        ...
        if (connections.length > 0) {
          Logger.debug(
            `Found ${
              connections.length
            } backlinks to ${vscode.workspace.asRelativePath(oldFileUri)}`
          );
        }
        for (const connection of connections) {
          ...
          Logger.debug(
            `Updating link in "${sourceFile}": "${connection.link.rawText}"`
          );
          ...
          if (sourceFilePath.startsWith(oldFolderPath)) {
            ...
            actualSourceUri = this.calculateNewFileUri(
              sourceFileUri,
              oldFolderUri,
              newFolderUri
            );
            Logger.debug(
              `  → Source file also moved to: ${vscode.workspace.asRelativePath(
                actualSourceUri
              )}`
            );
          }
          ...
          switch (connection.link.type) {
            case 'wikilink': {
              ...
              const { target: originalTarget } = MarkdownLink.analyzeLink(
                connection.link
              );

              // Check if the original wikilink identifier would still resolve correctly after the rename
              // If it's a simple identifier (no path separators), and it would still be unique, preserve it
              let shouldPreserveOriginal = false;

              // Only consider preserving simple identifiers (no path separators)
              // Path-based identifiers like "features/daily-notes" need to be updated when folders are renamed
              if (
                !originalTarget.includes('/') &&
                FoamWorkspace.isIdentifier(originalTarget)
              ) {
                // This is a simple identifier like [[note]] or [[daily-notes]]
                // Check if it would still resolve uniquely to our target after the rename

                // Get all resources that match this identifier (workspace is already updated)
                const candidatesAfterRename =
                  this.foam.workspace.listByIdentifier(originalTarget);

                // If there's exactly one candidate and it's our target, the simple identifier remains unambiguous
                if (
                  candidatesAfterRename.length === 1 &&
                  candidatesAfterRename[0].uri.isEqual(targetFoamUri)
                ) {
                  shouldPreserveOriginal = true;
                  Logger.debug(
                    `  → Preserving simple wikilink "${originalTarget}" as it remains unambiguous after rename`
                  );
                } else {
                  Logger.debug(
                    `  → Simple wikilink "${originalTarget}" has ${candidatesAfterRename.length} candidates, will update to avoid ambiguity`
                  );
                }
              } else {
                Logger.debug(
                  `  → Path-based wikilink "${originalTarget}" will be updated for folder rename`
                );
              }

              if (shouldPreserveOriginal) {
                // Don't update the wikilink - it's still valid
                Logger.debug(
                  `  → Wikilink in "${vscode.workspace.asRelativePath(
                    actualSourceUri
                  )}" preserved: "${connection.link.rawText}"`
                );
              } else {
                ...
                if (originalTarget.includes('/')) {
                  const wsFolder =
                    vscode.workspace.getWorkspaceFolder(newFileUri);
                  if (wsFolder) {
                    const rel = path
                      .relative(wsFolder.uri.fsPath, newFileUri.fsPath)
                      .replace(/\\/g, '/');
                    newLinkContent = rel.replace(path.extname(rel), '');
                  } else {
                    newLinkContent =
                      this.foam.workspace.getIdentifier(targetFoamUri);
                  }
                } else {
                  newLinkContent =
                    this.foam.workspace.getIdentifier(targetFoamUri);
                ...
                Logger.debug(
                  `  → Wikilink in "${vscode.workspace.asRelativePath(
                    actualSourceUri
                  )}" updated to: "${edit.newText}"`
                );
              }
              break;
            }
            case 'link': {
              // This is for standard markdown links [text](path)
              const targetFoamUri = fromVsCodeUri(newFileUri); // URI of the moved file (link target)
              const sourceDirectoryFoamUri =
                fromVsCodeUri(actualSourceUri).getDirectory();

              // Calculate the relative path string for the markdown link.
              const newLinkPath = targetFoamUri.relativeTo(
                sourceDirectoryFoamUri
              ).path;
              ...
              linksUpdatedForThisFile++;
              Logger.debug(
                `  → Markdown link in "${vscode.workspace.asRelativePath(
                  actualSourceUri
                )}" updated to: "${edit.newText}"`
              );
              break;
        ...
        const outgoingConnections = this.foam.graph.getLinks(
          fromVsCodeUri(oldFileUri)
        );
        ...
        if (outgoingConnections.length > 0) {
          Logger.debug(
            ...
            } outgoing links from ${vscode.workspace.asRelativePath(
              oldFileUri
            )}`
          );
        }
        for (const outgoingConnection of outgoingConnections) {
          ...
          Logger.debug(
            `Checking outgoing link: "${outgoingConnection.link.rawText}" (type: ${outgoingConnection.link.type}, target: "${target}")`
          );
          ...
          if (!isAbsolute(target) && target.startsWith('../')) {
            ...
            if (targetPath.startsWith(oldFolderPath)) {
              Logger.debug(
                `  → Skipping link to file within same renamed folder (handled by backlink processing)`
              );
              continue;
            }

            Logger.debug(
              `Processing outgoing relative link: "${outgoingConnection.link.rawText}"`
            );
            ...
            if (!workspaceRootPath) {
              Logger.warn(
                'Cannot process relative links: no workspace root found'
              );
              continue;
            }
            ...
            if (adjustedPath && adjustedPath !== target) {
              Logger.debug(
                `  → Adjusting relative path "${target}" to "${adjustedPath}"`
              );

              const edit = MarkdownLink.createUpdateLinkEdit(
                outgoingConnection.link,
                {
                  target: adjustedPath,
                }
              );
              ...
              linksUpdatedForThisFile++;
              Logger.debug(
                `  → Relative link updated in moved file: "${edit.newText}"`
              );
            } else {
              Logger.debug(
                `  → No adjustment needed for relative path "${target}"`
              );
            }
          } else {
            Logger.debug(
              `  → Skipping link (absolute: ${isAbsolute(
                target
              )}, starts with ../: ${target.startsWith('../')})`
            );
          }
        }
    ...
    Logger.debug(
      `Finished calculating link updates. Potential to update ${totalLinksUpdated} link(s) across ${renameEdits.size} file(s).`
    );
    if (progress) {
      progress.report({
        message: `Calculation complete: ${totalLinksUpdated} potential link updates found.`,
        increment: 100,
      });
    }
  ...
  private calculateAdjustedRelativePath(
    ...
    try {
      ...
      const oldRelativePath = path.relative(
        workspaceRootPath,
        path.dirname(oldFileUri.fsPath)
      );
      const newRelativePath = path.relative(
        workspaceRootPath,
        path.dirname(newFileUri.fsPath)
      );

      const oldDepth =
        oldRelativePath === '' ? 0 : oldRelativePath.split(path.sep).length;
      const newDepth =
        newRelativePath === '' ? 0 : newRelativePath.split(path.sep).length;

      const depthChange = newDepth - oldDepth;

      Logger.debug(
        `Depth calculation: old="${oldRelativePath}" (${oldDepth}), new="${newRelativePath}" (${newDepth}), change=${depthChange}`
      );
      ...
      if (newUpCount < 0) {
        Logger.warn(
          `Cannot adjust relative path ${originalRelativePath}: would result in negative up count`
        );
        return null;
      }
      ...
      Logger.debug(
        `Adjusted relative path "${originalRelativePath}" → "${adjustedPath}" (depth change: ${depthChange}, old up count: ${upCount}, new up count: ${newUpCount})`
      );
      return adjustedPath;
    } catch (error) {
      Logger.error(
        `Error calculating adjusted relative path for ${originalRelativePath}:`,
        error
      );
      return null;
    }
    ...
    try {
      ...
      for (const [uri, edits] of renameEdits.entries()) {
        try {
          ...
          result.warnings.push(
            `File not found, skipping link updates: ${uri.fsPath}`
          );
        }
      }
      ...
      if (filesToProcess.length > 0) {
        Logger.debug(
          `Applying consolidated workspace edit with changes to ${filesToProcess.length} files`
        );
        ...
        Logger.debug(
          `Saving ${filesToProcess.length} edited documents in parallel`
        );
        ...
        for (const saveResult of saveResults) {
          if (saveResult.success) {
            successfulEditsPaths.push(
              vscode.workspace.asRelativePath(saveResult.uri)
            );
          } else {
        ...
        Logger.debug(
          `Successfully applied ${actuallyAppliedLinksCount} link updates across ${
            filesToProcess.length
          } file(s): ${successfulEditsPaths.join(', ')}`
        );
      } else {
  ...
  private async revertFolderRename(
    ...
    Logger.debug(
      `Attempting to revert folder rename from "${vscode.workspace.asRelativePath(
        currentUri
      )}" back to "${vscode.workspace.asRelativePath(originalUri)}"`
    );
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Cannot revert: Current folder '${vscode.workspace.asRelativePath(
            currentUri
          )}' no longer exists`
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Cannot revert: Original location '${vscode.workspace.asRelativePath(
            originalUri
          )}' already exists`
        );
      } catch (statError) {
      ...
      Logger.debug(
        `Successfully reverted folder rename back to "${vscode.workspace.asRelativePath(
          originalUri
        )}"`
      );
    } catch (error) {
