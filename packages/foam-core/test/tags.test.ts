...
describe('FoamTags', () => {
  it('Collects tags from a list of resources', () => {
    ...
    expect(tags.tags).toEqual({
      primary: [
        { title: 'Page A', uri: pageA.uri },
        { title: 'Page B', uri: pageB.uri },
      ],
      secondary: [{ title: 'Page A', uri: pageA.uri }],
      third: [{ title: 'Page B', uri: pageB.uri }],
    });
  });
  ...
  it('Updates an existing tag when a note is tagged with an existing tag', () => {
    ...
    const tags = FoamTags.fromWorkspace(ws);
    expect(tags.tags).toEqual({
      primary: [{ title: 'Page A', uri: page.uri }],
    });
    ...
    tags.updateTagsForUpdatedResource(taglessPage, newPage);

    expect(tags.tags).toEqual({
      primary: [
        { title: 'Page A', uri: page.uri },
        { title: 'Page B', uri: newPage.uri },
      ],
    });
  });
  ...
  it('Replaces the tag when a note is updated with an altered tag', () => {
    ...
    tags.updateTagsForUpdatedResource(page, pageEdited);

    expect(tags.tags).toEqual({
      new: [{ title: 'Page A', uri: page.uri }],
    });
  });

  it('Updates the metadata of a tag when the title is altered', () => {
    const ws = createTestWorkspace();
    ...
    expect(tags.tags).toEqual({
      primary: [{ title: 'Page A is altered', uri: page.uri }],
    });
  });
  ...
  it('Updates the metadata of a tag when the note is moved', () => {
    ...
    expect(tags.tags).toEqual({
      primary: [{ title: 'Page A', uri: pageEdited.uri }],
    });
  });
  ...
  it('Updates the metadata of a tag when a note is delete', () => {
    ...
    tags.updateTagsForRemovedResource(page);
