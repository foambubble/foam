...
const resolveHomepageRoute = (
  ...
  if (homepage instanceof URI) {
    return context.locations.get(homepage.path)?.href ?? null;
  }

  if (Resource.isResource(homepage)) {
    return context.locations.get(homepage.uri.path)?.href ?? null;
  }
  ...
  return context.locations.get(matchingNote.uri.path)?.href ?? null;
};
