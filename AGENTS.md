...
`<slug>` is the branch name minus its type prefix (`feature/`, `fix/`, `spike/`), so the active spec is the one matching the current branch. One branch and one PR carry a feature from spec to merge. Labelling an issue `agent:spec` opens that PR; from then on the stages run from `@claude` comments on it (`@claude plan`, `@claude implement`), because once the PR exists it is both the queue entry and the place to say what you want done differently. Not everything needs a spec — a bug whose failing test says it all doesn't. See [specs/README.md](specs/README.md) for the layout and the acceptance-criteria format.
