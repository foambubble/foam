...
export const hash = (text: string) => crypto.createHash('md5').update(text).digest("hex")

export const uriToSlug = (noteUri: URI): string => {
  // TODO hack don't merge with this
  const filename = noteUri.split('/').slice(-1)[0].split('.').slice(0, -1).join('.');
...
export const getUriViaRelative = (reference: URI, relativeSlug: string): URI => {
